...
class LLMAnalysisResult(BaseModel):
    ...
    strengths: List[str] = Field(
        default_factory=list, description="강점 분석 - 잘하고 있는 부분과 강점 리스트"
    )
    improvement_recommendations: List[ImprovementRecommendation] = Field(
        default_factory=list, description="개선 방향 - 우선순위별 구체적인 개선 제안"
    )
    role_suitability: Optional[Dict[str, str]] = Field(
        default=None,
        description="역할 적합성 평가 - 개발자 타입별 적합성 평가",
    )
    model_config = ConfigDict(
        extra="allow"
    )  # 동적 필드 허용 (언어별 정보: "python", "javascript" 등), 각 언어별 stack, level, exp 정보 포함
